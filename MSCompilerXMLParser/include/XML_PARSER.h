...
class MSCOMPILERXMLPARSER_API XML_PARSER
{
public:
	XML_PARSER();                                                  // C'tor
	...
	size_t Get_Attribute_Count();                                     // Return the number of attributes for the current node
	CString & Get_Attribute_Name(size_t index);                       // Return the attribute name  for the nth attribute
	CString & Get_Attribute_Value(size_t index);                      // Return the attribute value for the nth attribute
...
private:
	...
	// Attrib Values
	//
	CArray<CString> AttribNames;
	CArray<CString> AttribValues;
	int attrib_index;
	...
	MSXML2::tagDOMNodeType Is_MSXML_Node(MSXML2::IXMLDOMNodePtr pChild);                  // Return NULL,NODE_ELEMENT,NODE_TEXT or NODE_CDATA_SECTION

	// ChainTag methods management
	//
	void ChainTag_Add(const CString & val);
